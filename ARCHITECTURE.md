...
thermostats (the non-Netatmo developer-cloud line). `iot_class:
...
| `webhook.py` | 99 | `X8000WebhookHandler` — validates push, fans out to coordinators |
| `config_flow.py` | 598 | 3-step user flow (creds → auth code → select thermostats) + reconfigure menu (external URL / renew credentials) |
| `climate.py` | 452 | `X8000Climate` — the thermostat entity |
| `sensor.py` | 803 | 7 per-device sensors + singleton diagnostics (API count, skips, polling tier, projected calls, token expiry) |
| `binary_sensor.py` | 88 | `X8000AuthProblemBinarySensor` — auth-health (PROBLEM) |
...
`PLATFORMS` (`__init__.py`): climate, sensor, binary_sensor, select, number,
switch, button.
...
- `DEFAULT_PASSIVE_MULTIPLIER` (4) — a user knob (`passive_multiplier`),
...
- Adaptive-throttle tiers are **derived** from the two user knobs, not fixed:
  ...
  quota 500 / 4×) the enforced values are 100/40 (thresholds) and 30/60
  (Economy active/passive minutes).
...
- `exchange_code_for_tokens`, `refresh_access_token` (retry/backoff, shared
  session), with typed errors: `TokenInvalidGrantError` (permanent, drives
  reauth) vs `TokenTransientError` (retriable). Refresh-token fallback preserved.

## 6. api.py — `X8000Api` (`api.py`)
...
- Request loop status handling: 401 → locked refresh (`:241`), 429 →
  `RateLimitError` (`:262`), 5xx → retry. The double-checked lock compares
  against a `sent_token` snapshot.
- Exceptions: `X8000ApiError` (base), `RateLimitError`, `AuthError`,
  `AuthBrokenError`, `AuthRetryableError`.
- Command write: `set_chronothermostat_status(plant_id, topology_id, payload)`
  ...
  are **not** called on removal — the C2C subscription is intentionally left in
...
## 7. coordinator.py — `X8000Coordinator` (`coordinator.py`)
...
- `_async_update_data:` (adaptive poll loop):
  ...
  - typed excepts `:255–268`; generic safety-net `:270–279`. The inline 429
    abort is shielded from the generic net by an `except UpdateFailed: raise`.
...
- `apply_optimistic(topology_id, patch)` → merges a command payload into
  `data[topology_id]` so the post-command re-parse reads the new state.
...
- `X8000WebhookHandler.handle_webhook` — validates dict, filters keys,
...
- `async_step_reconfigure`: **menu** with three self-contained sub-flows —
  ...
  - Requires HA ≥ 2024.4 (reconfigure flow); `MIN_REQUIRED_HA_VERSION` matches.
- `single_instance_allowed`. Stores `access_token_expires_on` (datetime) in
  ...
  external_url, device selection) is user-editable via the reconfigure menu.

## 10. Entities

### climate.py — `X8000Climate` (`climate.py`)
...
- Optimistic set + revert-on-failure for hvac_mode/temperature/preset. On success
  ...
  optimistic value with the stale cache.
...
### sensor.py (`sensor.py`)
- `X8000BaseSensor` + `_get_nested_value` safe path walker (`:176`).
- Per device: Temperature, Humidity, TargetTemperature (all `MEASUREMENT`,
  LTS-enabled), Mode (`MODE_MAP`), Status (`X8000LoadStateSensor`,
  `LOAD_STATE_MAP`), BoostTimeRemaining (0 when inactive), Program,
  `X8000DeviceApiCountSensor`.
- Singletons: `X8000GlobalApiCountSensor` (`:514`, `TOTAL_INCREASING`,
  DIAGNOSTIC, listens to every coordinator tick), `X8000SkippedPollsSensor`
  (`:568`), `X8000TokenExpirySensor` (`:620`), `X8000PollingTierSensor`
  (`:684`), `X8000ProjectedCallsSensor` (`:749`).
- `X8000DeviceApiCountSensor.native_value:811` →
  `api.usage_stats.get(topology_id, 0)`.

### binary_sensor.py (`binary_sensor.py`)
- `X8000AuthProblemBinarySensor` (device class PROBLEM) — **on** only when
  `api.auth_broken` (permanent failure driving reauth); always `available`.

### select.py (`select.py`)
- `X8000BoostSelect` options `["off","30","60","90"]`; heuristically infers the
  current option from `activationTime` (`_update_state_from_coordinator:163`).
- `X8000ProgramSelect` options = program names; optimistic write + revert (both
  merge into the coordinator cache via `apply_optimistic`).

### number.py (`number.py`)
- `X8000BaseNumber._update_config_entry:111` — writes `entry.options` (no
...
- `UpdateInterval` (shows `normal_interval`, not the cool-down penalty),
  `CoolDown` (force_refresh=True), `Debounce`, `DailyQuota`, `PassiveMultiplier`.

### switch.py (`switch.py`)
- `X8000NotifyErrorsSwitch`, `X8000SmartPollingSwitch` — mutate coordinator +
  persist to options.

### button.py (`button.py`)
- `X8000ForceTokenButton` → `coordinator.async_force_token_refresh()`. Always
