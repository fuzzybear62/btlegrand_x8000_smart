#!/usr/bin/env bash
#
# update_btlegrand_x8000.sh — update the btlegrand_x8000 integration on Home Assistant
# from the public GitHub repo.
#
# Run this in the Home Assistant terminal (SSH/Terminal add-on).
# Place it at /root/config/update_btlegrand_x8000.sh and run:  bash update_btlegrand_x8000.sh
#
# Options:
...
REPO_URL="https://github.com/fuzzybear62/btlegrand_x8000_smart.git"
CONFIG_DIR="/root/config"
DEST="${CONFIG_DIR}/custom_components/btlegrand_x8000"
SRC="${CONFIG_DIR}/.btlegrand_x8000_src"          # persistent clone, reused across updates
SUBPATH="custom_components/btlegrand_x8000"        # integration path inside the repo
BAK_GLOB="${CONFIG_DIR}/btlegrand_x8000.bak.*"
...
log() { echo "[update_btlegrand_x8000] $*"; }
...
if [ "${DO_BACKUP}" -eq 1 ] && [ -d "${DEST}" ]; then
  BAK="${CONFIG_DIR}/btlegrand_x8000.bak.$(date +%Y%m%d-%H%M%S)"
...
log "Deployed btlegrand_x8000 @ ${COMMIT}"
