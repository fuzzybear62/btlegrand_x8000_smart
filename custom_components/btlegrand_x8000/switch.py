...
class BticinoSmartPollingSwitch(CoordinatorEntity, SwitchEntity):
    ...
    If ON: Thermostats that are OFF (Passive) are polled less frequently (e.g. 4x slower).
    If OFF: All thermostats are polled at the same standard interval.
    """

    _attr_has_entity_name = True
    _attr_name = "Smart Energy Saving"
    _attr_entity_category = EntityCategory.CONFIG
    _attr_device_class = SwitchDeviceClass.SWITCH
    ...
    @property
    def available(self) -> bool:
        """Always available to allow configuration even during errors."""
        return True

    @property
    def is_on(self) -> bool:
        """Return True if smart polling is enabled."""
        return self.coordinator.smart_polling_enabled

    @property
    def icon(self) -> str:
        """Dynamic icon: Leaf for saving (ON), Speedometer for full speed (OFF)."""
        return "mdi:leaf" if self.is_on else "mdi:speedometer"
    ...
    async def _update_setting(self, enabled: bool) -> None:
        """Helper to update setting in memory and disk."""
        _LOGGER.info("User changed Smart Energy Saving to %s", "ON" if enabled else "OFF")

        # 1. Update Coordinator Memory
        self.coordinator.smart_polling_enabled = enabled
        ...
        # 3. Persist to Disk
        new_options = dict(self.coordinator.entry.options)
        new_options[CONF_SMART_POLLING] = enabled
