...
class BticinoUpdateIntervalNumber(BticinoBaseNumber):
    ...
    def native_value(self) -> float:
        ...
        FIX: We return 'self.coordinator.normal_interval' instead of the dynamic 'update_interval'.
        This ensures the UI always shows the user's setting (e.g. 15), not the temporary 
        cool down penalty (e.g. 60), preventing UI 'bouncing'.
        """
...
class BticinoCoolDownNumber(BticinoBaseNumber):
    ...
    @property
    def native_value(self) -> float:
        """Return the CONFIGURED cool_down_interval in minutes."""
...
class BticinoDailyQuotaNumber(BticinoBaseNumber):
    ...
    Default: 500.
    """
    _attr_name = "Daily API Quota"
    _attr_icon = "mdi:chart-histogram"
    _attr_key = CONF_BTLG_DAILY_QUOTA
    _attr_native_unit_of_measurement = "calls"
    
    # Boundaries from const.py
    _attr_native_min_value = MIN_BTLG_DAILY_QUOTA
    _attr_native_max_value = MAX_BTLG_DAILY_QUOTA
    # Larger step for better UX with wide ranges (100-10000)
    ...
    async def async_set_native_value(self, value: float) -> None:
        """Update the value."""
        new_quota = int(value)
        _LOGGER.info("User changed Daily API Quota to %s calls", new_quota)
